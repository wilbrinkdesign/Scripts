...
Function Project-Start
{
    ...
    # Paden handleidingen
    $PadLogoGuide = "$($env:OneDrive)\Wilbrink Design\Documents\Logo guide.pdf"
    $PadLogoGids = "$($env:OneDrive)\Wilbrink Design\Documents\Logo gids.pdf"
...
Function Project-RenameFiles
{
    ...
    {
        # Vraag alle files op in alleen de sub folders
        $RGB = Get-ChildItem $Pad\Logo\Files\*\* | where { $_.GetType().Name -eq "FileInfo" -and $_.FullName -notlike "*-rgb*" -and $_.FullName -notlike "*-cmyk*" }
    }
